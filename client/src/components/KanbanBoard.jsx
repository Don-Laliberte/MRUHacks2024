...
export default function KanbanBoard() {
  ...
  const totalColumnWidth = columnCount * COLUMN_WIDTH + (columnCount - 1) * COLUMN_GAP;
    ...
    <Container maxWidth={false} disableGutters ref={containerRef}>
      <Box sx={{ 
        ...
        <Box sx={{ 
          ...
          {Object.keys(columns).length < 6 && Object.keys(columns).length > 0 && (
            <Fab color="primary" size="large" aria-label="add" onClick={addColumn}>
              <AddIcon sx={{ fontSize: 36 }} />
            </Fab>
          )}
          {Object.keys(columns).length > 0 && (
            <Fab color="secondary" size="large" aria-label="remove" onClick={removeColumn}>
        ...
        >
          <Box sx={{ width: 300, p: 2 }}>
            ...
            {selectedCard && (
              <form onSubmit={(e) => {
                e.preventDefault();
                updateCard(selectedCard.columnId, selectedCard.id, selectedCard);
              }}>
                <TextField
                  fullWidth
                  label="Title"
                ...
                <TextField
                  ...
                  onChange={(e) => setSelectedCard({...selectedCard, timeEstimate: e.target.value})}
                  margin="normal"
                />
                <Box sx={{ mt: 2, display: 'flex', justifyContent: 'space-between' }}>
                  ...
                  <Button 
                    ...
                    color="error" 
                    Label="Delete"
                    onClick={() => deleteCard(selectedCard.columnId, selectedCard.id)}
                  >
                    Delete
                  </Button>
                </Box>
